# Speedtest Monitor
Scripts to collect internet speed metrics using `speedtest-cli`, store them locally en CSV, or push them to InfluxDB for dashboards.
...
## Uso rápido
- CSV local: `python -m app.speedtest` (genera/append en `CSV_PATH`, por defecto `/home/pi/speedtest/speedtest.csv`, ajusta ruta si usas otro OS).
- Enviar a InfluxDB: configura las variables de entorno `INFLUX_HOST`, `INFLUX_PORT`, `INFLUX_USER`, `INFLUX_PASSWORD`, `INFLUX_DB` y ejecuta `python -m app.speedtestdb`.
- Ejecución periódica: programa con `cron`/Task Scheduler según tu plataforma.
- Con Docker: `docker compose build` y `docker compose up -d` (usa un `.env` para InfluxDB, CSV y credenciales). Imagen por defecto ejecuta `python -m app.speedtestdb`; puedes sobrescribir `command` a `python -m app.speedtest` para CSV.
- Stack en Docker Compose:
  ...
  - `speedtest` (este proyecto) que envía datos a InfluxDB cada `INTERVAL_SECONDS`.
...
- Tipado opcional: `mypy` (`mypy app tests`) para verificar anotaciones.
...
- Bandit: `bandit -r app tests` para análisis estático de código Python.
...
- `mypy app tests` para validación de tipos.
- `gitleaks detect`, `bandit -r app tests` y `trivy fs .` para seguridad.
